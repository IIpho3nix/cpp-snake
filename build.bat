...
for %%f in (src\*.cpp src\*.c %custom_src%) do (
    echo Compiling %%~nxf...
    g++ -O3 -flto -fno-exceptions -s -ffunction-sections -fdata-sections -I"%raylib_path%\include" -I"src" -I"src/include" %custom_include% -c %%f -o build\%%~nf.o
)
...
if "%link%"=="dynamic" (
    ...
    g++ -O3 -flto -fno-exceptions -s -Wl,--gc-sections -mwindows -L"%raylib_path%\lib" -L"src/lib" build\*.o -o build\%name%.exe -l:raylib.dll -lgdi32 -lwinmm
    copy raylib\lib\raylib.dll build\raylib.dll
) else (
    echo Static linking raylib
    g++ -O3 -flto -fno-exceptions -s -Wl,--gc-sections -mwindows -L"%raylib_path%\lib" -L"src/lib" build\*.o -o build\%name%.exe -lraylib -lgdi32 -lwinmm
)
